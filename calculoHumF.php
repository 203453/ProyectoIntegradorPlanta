<?php
	...
	$result = mysqli_query($conn->conectarDb(),"SELECT `humF` FROM muestras");
	$humsF = $result->fetch_all(MYSQLI_ASSOC);


    foreach($humsF as $humF):
        array_push($datosArray, $humF['humF']);
    endforeach;
    ...
    for ($i = 0; $i < $K; $i++) {
        ...
        $result = "INSERT INTO `clasesHumF`(`class`, `limInf`, `limSup`, `frec`, `frecRelat`, `frecAcum`, `marcaClase`, `frecComp`, `limInfExact`, `limSupExact`) VALUES ('$class','$limInf','$limSup','$frec','$frecRelat','$frecAcum','$marcaClase','$frecComp','$limInfExact','$limSupExact')";
