<?php
	...
	$result = mysqli_query($conn->conectarDb(),"SELECT `temp` FROM muestras");
	$temps = $result->fetch_all(MYSQLI_ASSOC);


    foreach($temps as $temp):
        array_push($datosArray, $temp['temp']);
    endforeach;
    ...
    for ($i = 0; $i < $K; $i++) {
        ...
        $result = "INSERT INTO `clasesTemp`(`class`, `limInf`, `limSup`, `frec`, `frecRelat`, `frecAcum`, `marcaClase`, `frecComp`, `limInfExact`, `limSupExact`) VALUES ('$class','$limInf','$limSup','$frec','$frecRelat','$frecAcum','$marcaClase','$frecComp','$limInfExact','$limSupExact')";
