<?php
    ...
    require_once('calculoHum.php');

	$result = mysqli_query($conn->conectarDb(),"SELECT * FROM muestras");
	$customers = $result->fetch_all(MYSQLI_ASSOC);
    mysqli_free_result($result);
    $result = mysqli_query($conn->conectarDb(),"SELECT * FROM muestraHum");
	$customers2 = $result->fetch_all(MYSQLI_ASSOC);
    mysqli_free_result($result);
    $result = mysqli_query($conn->conectarDb(),"SELECT * FROM clasesHum");
	$customers3 = $result->fetch_all(MYSQLI_ASSOC);
    mysqli_free_result($result);

    require_once('dispersionHum.php');
    ...
    if ($zCalculada > -1.96 || $zCalculada < 1.96) {
        ...
        $auxR = 'nula';
        $auxB = 'cierto';
        $auxC = 'La humedad promedio es de '.$mediaNoA;
    } else {
        $auxZ = 'H1';
        $auxR = 'alternativa';
        $auxB = 'falso';
        $auxC = 'La humedad promedio no es de '.$mediaNoA;
    }
...
<body>
    ...
    <center><h5 class="datosTitle">HUMEDAD</h5></center>
    ...
    <div class="container well">
        ...
        <div class="caja">
            ...
            <div class="datosMuestra" style="height: 600px; width: 350px; overflow-y: auto; text-align: center;">
                <div class="texto">
                    <h5 class="datosTitle">DATOS MUESTRA HUMEDAD</h5>
                    <br>
                    <h6>N =  <?php echo $N2?></h6>
                    ...
                    <br>
                    <h6>PLANTEAMIENTO:<br><br> Un agricultor afirma que, al monitorear y recabar información sobre la humedad de una planta, está es en promedio de <?php echo $mediaNoA; ?>. 
                    Suponga que se toma una muestra aleatoria de 40 datos para probar si la humedad coincide con lo anterior.</h6>
                    <h6>La humedad en promedio de la muestra fue de <?php echo $mediaNoA2 ?> con una desviación estándar de <?php echo $desvEstandarMuestra2; ?>.</h6>
                    <h6>Pruebe una hipótesis nula de que la humedad promedio es de <?php echo $mediaNoA; ?>, contra la hipótesis alternativa de que el promedio no es de <?php echo $mediaNoA; ?>. Utilice un nivel de significancia de 0.5 (5%).</h6>
                    <h6>Hipótesis nula(Ho): La humedad promedio es de <?php echo $mediaNoA; ?></h6>
                    <h6>Hipótesis alternativa(H1): La media de la muestra nos dice que la humedad promedio es <?php echo $auxH; ?> a lo mencionado antes.</h6>
